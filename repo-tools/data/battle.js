...
const { logMissingFile, hasFile, gitio } = require("../utilities.js")
...
module.exports = async function() {
	...
	return Promise.all(categoryDirectories.map(async categoryDir => {
		...
		return Promise.all(await cleanDirectories.map(async animDir => {
			...
			return Promise.all(weaponDirs.map(async (weaponDir) => {
				...
				weapon.path = `${ROOT_DIR_SLUG}/${categoryDir}/${animDir}/${weaponDir}`
